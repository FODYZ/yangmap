...
def test_le_prefixe_de_module_du_premier_segment_est_retire():
    """C1 — `/nokia-state:state/...` becomes `/state/...`."""
...
def test_un_prefixe_en_milieu_de_chemin_est_retire_mais_pas_perdu():
    ...
    The conventional gNMI path doesn't carry module prefixes, but the
    canonical xpath keeps them: nothing is lost, and a consumer that needs
    the qualified form can reconstruct it.
    """
    ...
    assert c.xpath == x, "the canonical xpath must remain intact"


def test_les_cles_attendent_une_valeur():
    """C3 — `[router-name]` becomes `[router-name=?]`."""
...
def test_une_liste_a_cles_multiples_les_conserve_toutes():
    """C4 — the real case `[ip-address][mac-address][pppoe-session-id]`."""
    c = analyser(
...
def test_une_cle_deja_valuee_est_laissee_intacte():
    """A caller who already knows what it wants shouldn't be corrected."""
...
def test_la_normalisation_est_pure():
    """C6 — same inputs, same outputs, no state."""
...
def test_les_segments_sont_eclates_en_mots_pour_l_indexation():
    """`route-table` must be findable by "route" as well as by "table"."""
